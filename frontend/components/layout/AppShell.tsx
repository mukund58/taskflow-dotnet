...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen">
      {/* Header — transparent on landing, solid on inner pages */}
      <header
        className={cn(
          "border-b border-[var(--color-border)]/80 backdrop-blur",
          isLanding
            ? "absolute inset-x-0 top-0 z-30 border-transparent bg-transparent"
      ...
      <main>
        {isLanding ? (
          // Full-bleed landing — no container, no padding
          children
        ) : (
